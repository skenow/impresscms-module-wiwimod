...
include_once ICMS_MODULES_PATH . '/' . $wikiModDir . '/class/wiwiProfile.class.php';
...
function swiki_search($queryarray, $andor, $limit, $offset, $userid) {
	...
	while ($myrow = icms::$xoopsDB->fetchArray($result)) {
		$prf->load($myrow['prid']);
		if ($prf->canRead()) {
			$items[] = array ('title' => $myrow['title'],'link' => 'index.php?page=' . $myrow['keyword'],'time' => strtotime($myrow['modified']),'uid' => $myrow['userid'],'image' => '../../images/quote.gif'
			);
		}
	}
