 ...
 * @since		1.00
 * @version		$Id: plugin.tag.php 21035 2011-03-12 22:45:39Z skenow $
 * @package		module::tag
 */
...
function wiki_tag_iteminfo(&$items) {
	...
	foreach (array_keys($items) as $cat_id) {
		...
		foreach (array_keys($items[$cat_id]) as $item_id) {
			...
			$taggedPage =  new WiwiRevision(NULL, 0, $item_id);
...
/* This will create a function with a name based on the installation directory, if it is not wiki */
$myInstallDir = basename(dirname(dirname(__FILE__)));
if (!function_exists($myInstallDir . '_tag_iteminfo')) {
	$myfunc = 'function '. $myInstallDir . '_tag_iteminfo(&$items) { return wiki_tag_iteminfo($items);}';
	eval($myfunc);
}
/**
