 ...
 * @version $Id: blocksadmin.inc.php 21035 2011-03-12 22:45:39Z skenow $
 */
if (!is_object($xoopsUser) || !is_object($xoopsModule) || !$xoopsUser->isAdmin($xoopsModule->mid())) {
	exit('Access Denied');
}

include_once ICMS_ROOT_PATH . '/class/xoopsblock.php'; /* future: '/kernel/block.php' */
$file = ICMS_ROOT_PATH . '/modules/system/admin/blocksadmin/class/blocksadmin.php';
if (file_exists($file)){
...
function icms_update_block($bid, $bside, $bweight, $bvisible, $btitle, $bcontent, $bctype, $bcachetime, $bmodule, $options=array()) {
	...
	if ($myblock->store() !== FALSE) {
		...
		foreach ($bmodule as $bmid) {
			...
			$mid = $page[0];
			$pageid = $page[1];
			$sql = "INSERT INTO " . $db->prefix('block_module_link') . " (block_id, module_id, page_id) VALUES ('" . (int) $bid . "', '" . (int) $mid . "', '" . (int) $pageid . "')";
			$db->query($sql);
		}
		include_once ICMS_ROOT_PATH . '/class/template.php';
		$xoopsTpl = new XoopsTpl();
