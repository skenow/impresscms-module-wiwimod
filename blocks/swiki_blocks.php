...
include_once $wikiModPath . '/class/wiwiRevision.class.php';
...
function swiki_listpages($options) {
	...
	$field = in_array($options[1], array ('createdate','lastmodified','revisions','lastviewed','views'
	), true) ? (string) $options[1] : '';
	$sort = in_array($options[2], array ('ASC','DESC'
	), true) ? (string) $options[2] : '';
	$display = in_array($options[3], array ('compact','light','full'
	), true) ? (string) $options[3] : '';
...
function swiki_listpages_blockedit($options) {
	...
	$form .= _MB_SWIKI_FIELD_DESC . "&nbsp;<select name='options[1]' size='1'><option value='createdate'" . ($options[1] == "createdate" ? "selected='selected'>" : ">") . _MB_SWIKI_CREATE_DATE . "</option><option value='lastmodified'" . ($options[1] == "lastmodified" ? "selected='selected'>" : ">") . _MB_SWIKI_MODIFIED_DATE . "</option><option value='lastviewed'" . ($options[1] == "lastviewed" ? "selected='selected'>" : ">") . _MB_SWIKI_LASTVIEWED_DATE . "</option><option value='revisions'" . ($options[1] == "revisions" ? "selected='selected'>" : ">") . _MB_SWIKI_REVISIONS . "</option><option value='views'" . ($options[1] == "views" ? "selected='selected'>" : ">") . _MB_SWIKI_VIEWS . "</option></select><br />";
	$form .= _MB_SWIKI_SORT_OPTION . "&nbsp;<select name='options[2]' size='1'><option value='ASC'" . ($options[2] == "ASC" ? "selected='selected'>" : ">") . _MB_SWIKI_ASCENDING . "</option><option value='DESC'" . ($options[2] == "DESC" ? "selected='selected'>" : ">") . _MB_SWIKI_DESCENDING . "</option></select><br />";
	$form .= _MB_SWIKI_DISPLAY_MODE . "&nbsp;<select name ='options[3]' size='1'><option value='compact'" . ($options[3] == "compact" ? "selected='selected'>" : ">") . _MB_SWIKI_DISPLAY_COMPACT . "</option><option value='light'" . ($options[3] == "light" ? "selected='selected'>" : ">") . _MB_SWIKI_DISPLAY_LIGHT . "</option><option value='full'" . ($options[3] == "full" ? "selected='selected'>" : ">") . _MB_SWIKI_DISPLAY_FULL . "</option></select><br />";
