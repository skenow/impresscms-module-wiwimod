...
default:
	...
	echo '}</script>';
	echo '<table><tr><td>';
	echo "<table cellpadding='4' cellspacing='1'><tr><th colspan='2'>"._AM_SWIKI_SELECTACL_BOX."</th></tr>";
	echo '<tr><td >'._AM_SWIKI_SELECTACL_TXT.'</td>';
	echo '<td ><select name=profile onchange="loadPrf(this);">';
	echo '<option value="0">'._AM_SWIKI_SELECTACL_OPT.'</option>';
	foreach ($prflst as $key=>$value) {
		echo '<option value="'.$key.($prid != $key ? '">' : '" selected>').$value.'</option>';
	}
	echo '</select></td></tr>';
	echo '<tr><td >'._AM_SWIKI_CREATEACL_TXT.'</td>';
	echo '<td ><input type=button value="'._AM_SWIKI_CREATEACL_BTN.'" onclick="location.href=\'acladmin.php\'"/>';
	echo '</td></tr></table><br>';
