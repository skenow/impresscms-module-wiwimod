...
$allowed_get = array('profile' => 'int', 'newprf' => 'int');
$clean_GET = swiki_cleanVars($_GET, $allowed_get);
extract($clean_GET);

$allowed_postvars = array('prid' => 'int', 'prf_name' => 'plaintext', 'prf_readers' => 'array', 'prf_writers' => 'array', 'prf_administrators' => 'array', 'prf_commentslevel' => 'int', 'prf_historylevel' => 'int', 'op' => 'string');
if (!empty($_POST)) {
...
if (in_array($op, $valid_ops, true)) {
	switch ($op) {
		...
		default:
			...
			echo '<script>function loadPrf(ele) { ' . '		var prf= ele.value;  ' . '		if (prf != 0) location.href="acladmin.php?op=edit&profile="+prf;' . '}</script>' . '<table><tr><td>' . "<table cellpadding='4' cellspacing='1'><tr><th colspan='2'>" . _AM_SWIKI_SELECTACL_BOX . "</th></tr>" . '<tr><td >' . _AM_SWIKI_SELECTACL_TXT . '</td>' . '<td ><select name=profile onchange="loadPrf(this);">' . '<option value="0">' . _AM_SWIKI_SELECTACL_OPT . '</option>';
			foreach ($prflst as $key => $value) {
				echo '<option value="' . $key . ($prid != $key ? '">' : '" selected>') . $value . '</option>';
			}
			echo '</select></td></tr>' . '<tr><td >' . _AM_SWIKI_CREATEACL_TXT . '</td>' . '<td ><input type=button value="' . _AM_SWIKI_CREATEACL_BTN . '" onclick="location.href=\'acladmin.php\'"/>' . '</td></tr></table><br>';
			...
			$aclComments->addOptionArray(array(0 => _AM_SWIKI_COMMENTS_NONE_OPT, _WI_READ => _AM_SWIKI_READERS_OPT, _WI_WRITE => _AM_SWIKI_WRITERS_OPT, _WI_ADMIN => _AM_SWIKI_ADMINISTRATORS_OPT));
			$aclHistory = new icms_form_elements_Select(_AM_SWIKI_HISTORY_FLD, 'prf_historylevel', $prf->historylevel);
			$aclHistory->addOptionArray(array(0 => _AM_SWIKI_HISTORY_NONE_OPT, _WI_READ => _AM_SWIKI_READERS_OPT, _WI_WRITE => _AM_SWIKI_WRITERS_OPT, _WI_ADMIN => _AM_SWIKI_ADMINISTRATORS_OPT));
		...
		case 'save':
			if (isset($_POST)) {
				...
				if (isset($prf_administrators))
					...
					$prf->administrators = array();
				$prf->commentslevel = $prf_commentslevel;
				$prf->historylevel = $prf_historylevel;

				$success = $prf->store();
				redirect_header('acladmin.php?op=edit&amp;profile=' . $prid, 2, ($success ? _AM_SWIKI_PRFSAVESUCCESS_MSG : _AM_SWIKI_PRFSAVEFAILED_MSG));
				break;
			}
		...
		case 'confirmdelete':
			if (isset($profile)) {
				...
				redirect_header('acladmin.php', 2, _AM_SWIKI_ERRDELETE_MSG);
